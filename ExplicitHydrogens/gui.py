...
# Ajusta o diretório de trabalho corretamente
...
def get_output_folder():
    try:
        with open(resource_path(CONFIG_FILE)) as f:
            config = json.load(f)
...
def show_waiting_window():
    ...
    waiting_window.geometry("300x120")
...
def run_program_thread():
    try:
        config = {
            ...
            "PARAM_FILE": resource_path("parameters.json"),
            ...
            "EXPLICIT_HYDROGENS": explicit_h_var.get(),
...
def open_output_folder():
    folder = os.path.abspath(get_output_folder())
    if not os.path.exists(folder):
        messagebox.showwarning("Directory not found", f"The directory {folder} was not created yet.")
        return
    try:
...
# GUI Layout
root = tk.Tk()
root.title("ForMileS v2.6 (Explicit-H)")
root.geometry("420x760")

tk.Label(root, text="Molecular Formula (supports explicit H):").pack()
formula_var = tk.StringVar(value="C6H12O2")
tk.Entry(root, textvariable=formula_var).pack()

tk.Label(root, text="Basic Molecular Scaffold (SMILES, explicit H OK):").pack()
...
output_dir = tk.StringVar(value=os.path.abspath("."))
tk.Entry(root, textvariable=output_dir, width=45).pack()
tk.Button(root, text="Select Folder", command=lambda: output_dir.set(filedialog.askdirectory())).pack(pady=2)

tk.Label(root, text="Structural Configurations").pack(pady=5)
...
tk.Label(root, text="Maximum Triple Bonds:").pack()
...
# New options
explicit_h_var = tk.BooleanVar(value=True)
tk.Checkbutton(root, text="Enable explicit hydrogens (formula & SMILES)", variable=explicit_h_var).pack(pady=4)
benchmark_var = tk.BooleanVar(value=True)
tk.Checkbutton(root, text="Benchmark time & peak memory", variable=benchmark_var).pack(pady=2)
